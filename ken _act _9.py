...
ACTIVITIES_DIR = Path.home() / "AppData" / "Local" / "PullDownMenu"
ACTIVITIES_DIR.mkdir(parents=True, exist_ok=True)
ACTIVITIES_FILE = ACTIVITIES_DIR / "activities_data.json"

activities_log = []

# Globals assigned inside show_main_editor but used by module-level functions
window = None
text_editor = None
statusbar = None
...
def show_activities():
    ...
    # Delete button in list panel (defined after action_bar so delete_selected exists)
    def delete_selected():
        sel = listbox.curselection()
        if not sel:
            messagebox.showinfo("Activities", "Please select a file first.")
    ...
    tk.Button(list_frame, text="🗑 Delete Selected", command=delete_selected,
...
def save_all():
    """Save All — instantly saves BOTH script.py and notes.txt directly to Activities."""
    content = text_editor.get("1.0", tk.END)
...
def show_main_editor():
    ...
    def file_popup(event=None):
        w = event.widget if event else file_btn
    ...
    file_btn = tk.Button(toolbar, text="📁  File", bg="#1e3a5f", fg="#93c5fd",
                         ...
                         **btn_cfg, command=lambda: file_popup())
    ...
    def activities_popup(event=None):
        ...
        if not activities_log:
            ...
            activities_menu_popup.add_command(label="  ── MIDTERM ACTIVITIES ──", state=tk.DISABLED)
            if midterm_entries:
            ...
            activities_menu_popup.add_command(label="  ── FINAL ACTIVITIES ──", state=tk.DISABLED)
            if final_entries:
            ...
            if other_entries:
                activities_menu_popup.add_separator()
                activities_menu_popup.add_command(label="  ── OTHER SAVED FILES ──", state=tk.DISABLED)
        ...
        w = event.widget if event else activities_btn
    ...
    activities_btn = tk.Button(toolbar, text="📂  Activities", bg="#4a1d0e", fg="#fb923c",
                               ...
                               command=lambda: activities_popup())
    ...
    def help_popup(event=None):
        w = event.widget if event else help_btn
    ...
    help_btn = tk.Button(toolbar, text="❓  Help", bg="#3b1f5e", fg="#c4b5fd",
                         ...
                         **btn_cfg, command=lambda: help_popup())
    ...
    statusbar = tk.Label(status_frame, text="  ✦  Ready  |  Pull Down Menu",
...
def start_login():
    ...
    user_entry.bind("<FocusIn>", on_user_focus)
...
def clear_fields():
    for e in [entry_number, entry_name, entry_course, entry_prelim, entry_midterm, entry_final]:
        e.delete(0, tk.END)
    result_text.set("")
...
tk.Label(frame, text="Grade Calculator", font=("Segoe UI", 18, "bold"), bg="#e8f2ff", fg="#1b3a75").grid(row=0, column=0, columnspan=2, pady=(0, 18), sticky="ew")

lo = {"font": ("Segoe UI", 10), "bg": "#e8f2ff", "fg": "#1b3a75"}
eo = {"bd": 2, "relief": "groove", "font": ("Segoe UI", 10)}

for text, row in [("Student Number:", 1), ("Student Name:", 2), ("Student Course:", 3), ("Prelim Grade:", 4), ("Midterm Grade:", 5), ("Final Grade:", 6)]:
    tk.Label(frame, text=text, **lo).grid(row=row, column=0, sticky="e", pady=7)

entry_number = tk.Entry(frame, **eo); entry_number.grid(row=1, column=1, pady=7, padx=(10,0), sticky="ew")
entry_name   = tk.Entry(frame, **eo); entry_name.grid(row=2, column=1, pady=7, padx=(10,0), sticky="ew")
entry_course = tk.Entry(frame, **eo); entry_course.grid(row=3, column=1, pady=7, padx=(10,0), sticky="ew")
entry_prelim = tk.Entry(frame, **eo); entry_prelim.grid(row=4, column=1, pady=7, padx=(10,0), sticky="ew")
entry_midterm= tk.Entry(frame, **eo); entry_midterm.grid(row=5, column=1, pady=7, padx=(10,0), sticky="ew")
entry_final  = tk.Entry(frame, **eo); entry_final.grid(row=6, column=1, pady=7, padx=(10,0), sticky="ew")

tk.Button(frame, text="Calculate", command=calculate_grade, bg="#4c8bf5", fg="white", relief="flat", font=("Segoe UI", 11, "bold"), padx=10, pady=8).grid(row=7, column=0, columnspan=2, pady=16, sticky="ew")
tk.Button(frame, text="Add Another Student", command=clear_fields, bg="#5ac18e", fg="white", relief="flat", font=("Segoe UI", 11, "bold"), padx=10, pady=8).grid(row=8, column=0, columnspan=2, pady=8, sticky="ew")

result_text = tk.StringVar()
tk.Label(frame, textvariable=result_text, justify="left", bg="#fff8c4", relief="sunken", padx=12, pady=12, anchor="w", font=("Segoe UI", 10)).grid(row=9, column=0, columnspan=2, pady=18, sticky="ew")
...
def clear_fields():
    for entry in [emp_no_entry, name_entry, rate_entry, days_entry, sss_entry, philhealth_entry, cash_entry]:
        entry.delete(0, tk.END)
    gross_var.set(""); deduct_var.set(""); net_var.set("")
...
tk.Label(root, text="PAYROLL SYSTEM", font=("Arial", 16, "bold"), bg="#f4f6f7").pack(pady=10)
...
emp_no_entry = tk.Entry(emp_frame, width=25); emp_no_entry.grid(row=0, column=1, pady=3)
tk.Label(emp_frame, text="Employee Name:").grid(row=1, column=0, sticky="w")
name_entry = tk.Entry(emp_frame, width=25); name_entry.grid(row=1, column=1, pady=3)
tk.Label(emp_frame, text="Rate per Day:").grid(row=2, column=0, sticky="w")
rate_entry = tk.Entry(emp_frame, width=25); rate_entry.grid(row=2, column=1, pady=3)
tk.Label(emp_frame, text="Days Worked:").grid(row=3, column=0, sticky="w")
days_entry = tk.Entry(emp_frame, width=25); days_entry.grid(row=3, column=1, pady=3)
...
sss_entry = tk.Entry(ded_frame, width=25); sss_entry.grid(row=0, column=1, pady=3)
tk.Label(ded_frame, text="PhilHealth:").grid(row=1, column=0, sticky="w")
philhealth_entry = tk.Entry(ded_frame, width=25); philhealth_entry.grid(row=1, column=1, pady=3)
tk.Label(ded_frame, text="Cash Advance:").grid(row=2, column=0, sticky="w")
cash_entry = tk.Entry(ded_frame, width=25); cash_entry.grid(row=2, column=1, pady=3)

result_frame = tk.LabelFrame(root, text="Payroll Summary", padx=10, pady=10)
result_frame.pack(padx=15, pady=5, fill="both")
gross_var = tk.StringVar(); deduct_var = tk.StringVar(); net_var = tk.StringVar()
...
def compute_grade():
    try:
        ...
        if 97 <= average <= 100: numerical="1.00"; remark="Excellent"
        elif 94 <= average < 97: numerical="1.25"; remark="Very Good"
        elif 91 <= average < 94: numerical="1.50"; remark="Very Good"
        elif 88 <= average < 91: numerical="1.75"; remark="Good"
        elif 85 <= average < 88: numerical="2.00"; remark="Above Average"
        elif 82 <= average < 85: numerical="2.25"; remark="Above Average"
        elif 79 <= average < 82: numerical="2.50"; remark="Average"
        elif 76 <= average < 79: numerical="2.75"; remark="Average"
        elif average >= 75:      numerical="3.00"; remark="Passing"
        elif 72 <= average < 75: numerical="3.25"; remark="Conditional"
        elif 69 <= average < 72: numerical="3.50"; remark="Conditional"
        elif 66 <= average < 69: numerical="3.75"; remark="Failed"
        ...
        tree.insert("", "end", values=(stud_id, name, course, subject, prelim, midterm, final, f"{average:.2f}", numerical, remark))
    except ValueError:
...
def clear_fields():
    for e in [entry_id, entry_name, entry_subject, entry_prelim, entry_midterm, entry_final]:
        e.delete(0, tk.END)
    combo_course.set("")
    avg_result.config(text=""); num_result.config(text=""); remark_result.config(text="")
...
def delete_record():
    ...
    if not sel:
        ...
        return
    if messagebox.askyesno("Delete Record", "Are you sure?"):
        tree.delete(sel)
...
tk.Label(root, text="STUDENT GRADING SYSTEM", font=("Arial",18,"bold"), bg="#00FFFF", fg="#1a3d7c").pack(pady=10)

frame_info = tk.LabelFrame(root, text="Student Information", font=("Arial",11,"bold"), padx=15, pady=10, bg="#f7fbff")
...
combo_course = ttk.Combobox(frame_info, values=["BSIT","BSCS","BSBA","BSED","BSCE","BSME","BSEE","BSCpE","BSTCM","BSES","BSMET","BSAP"], state="readonly")
...
frame_grades = tk.LabelFrame(root, text="Grades Input", font=("Arial",11,"bold"), padx=15, pady=10, bg="#f7fbff")
...
tk.Button(button_frame, text="Compute", width=15, bg="#2c7be5", fg="white", font=("Arial",10,"bold"), command=compute_grade).grid(row=0, column=0, padx=10)
tk.Button(button_frame, text="Clear", width=15, bg="#dc3545", fg="white", font=("Arial",10,"bold"), command=clear_fields).grid(row=0, column=1, padx=10)
tk.Button(button_frame, text="Delete Selected", width=15, bg="#ff8800", fg="white", font=("Arial",10,"bold"), command=delete_record).grid(row=0, column=2, padx=10)

frame_result = tk.LabelFrame(root, text="Result", font=("Arial",11,"bold"), padx=15, pady=15, bg="#f7fbff")
frame_result.pack(fill="x", padx=20, pady=10)
tk.Label(frame_result, text="Average Grade:", bg="#f7fbff").grid(row=0, column=0, sticky="w")
avg_result = tk.Label(frame_result, font=("Arial",11,"bold"), fg="blue", bg="#f7fbff"); avg_result.grid(row=0, column=1)
tk.Label(frame_result, text="Numerical Value:", bg="#f7fbff").grid(row=1, column=0, sticky="w")
num_result = tk.Label(frame_result, font=("Arial",11,"bold"), fg="green", bg="#f7fbff"); num_result.grid(row=1, column=1)
tk.Label(frame_result, text="Remarks:", bg="#f7fbff").grid(row=2, column=0, sticky="w")
remark_result = tk.Label(frame_result, font=("Arial",11,"bold"), fg="red", bg="#f7fbff"); remark_result.grid(row=2, column=1)

frame_table = tk.LabelFrame(root, text="Student Dataset Records", font=("Arial",11,"bold"), bg="#f7fbff")
...
def Database():
    ...
    cursor.execute("""CREATE TABLE IF NOT EXISTS member (
        ...
        address TEXT, province TEXT, username TEXT, password TEXT)""")

def Create():
    if "" in (FIRSTNAME.get(), LASTNAME.get(), GENDER.get(), ADDRESS.get(), PROVINCE.get(), USERNAME.get(), PASSWORD.get()):
        txt_result.config(text="Please complete all fields!", fg="red"); return
    Database()
    cursor.execute("INSERT INTO member (firstname,lastname,gender,address,province,username,password) VALUES (?,?,?,?,?,?,?)",
        (FIRSTNAME.get(), LASTNAME.get(), GENDER.get(), ADDRESS.get(), PROVINCE.get(), USERNAME.get(), PASSWORD.get()))
    conn.commit(); conn.close()
    txt_result.config(text="Data Created Successfully!", fg="green"); Read()
...
def Update():
    ...
    Database()
    cursor.execute("UPDATE member SET firstname=?,lastname=?,gender=?,address=?,province=?,username=?,password=? WHERE firstname=? AND lastname=?",
        (FIRSTNAME.get(), LASTNAME.get(), GENDER.get(), ADDRESS.get(), PROVINCE.get(), USERNAME.get(), PASSWORD.get(), values[0], values[1]))
    conn.commit(); conn.close()
    txt_result.config(text="Updated Successfully!", fg="blue"); Read()
...
def Delete():
    ...
    conn.commit(); conn.close()
    txt_result.config(text="Deleted Successfully!", fg="red"); Read()

def OnSelected(event):
    selected = tree.focus()
    values = tree.item(selected, "values")
    if not values: return
    FIRSTNAME.set(values[0]); LASTNAME.set(values[1]); GENDER.set(values[2])
    ADDRESS.set(values[3]); PROVINCE.set(values[4]); USERNAME.set(values[5]); PASSWORD.set(values[6])

def Exit():
    if tkMessageBox.askquestion("Exit", "Are you sure?") == "yes":
        root.destroy()

FIRSTNAME = StringVar(); LASTNAME = StringVar(); GENDER = StringVar()
ADDRESS = StringVar(); PROVINCE = StringVar(); USERNAME = StringVar(); PASSWORD = StringVar()

Top = Frame(root, bg="#34495e", height=50); Top.pack(fill=X)
Label(Top, text="Student CRUD System", font=("Arial", 20, "bold"), bg="#34495e", fg="white").pack()
...
Forms = Frame(Left, bg="#ecf0f1"); Forms.pack(pady=10)
Buttons = Frame(Left, bg="#ecf0f1"); Buttons.pack(pady=10)
...
fg = Frame(Forms, bg="#ecf0f1"); fg.grid(row=2, column=1)
Radiobutton(fg, text="Male", variable=GENDER, value="Male").pack(side=LEFT)
Radiobutton(fg, text="Female", variable=GENDER, value="Female").pack(side=LEFT)
...
province_combo["values"] = ("Bukidnon","Camiguin","Lanao del Norte","Misamis Oriental","Misamis Occidental","Davao del Sur","Davao del Norte","Zamboanga del Sur","Zamboanga del Norte","South Cotabato","Sultan Kudarat","Agusan del Norte","Agusan del Sur","Surigao del Norte","Surigao del Sur")
...
txt_result = Label(Buttons, bg="#ecf0f1"); txt_result.pack()
...
tree = ttk.Treeview(Right, columns=("Firstname","Lastname","Gender","Address","Province","Username","Password"), xscrollcommand=scrollbarx.set, yscrollcommand=scrollbary.set)
scrollbarx.pack(side=BOTTOM, fill=X); scrollbary.pack(side=RIGHT, fill=Y)
scrollbarx.config(command=tree.xview); scrollbary.config(command=tree.yview)
for col in ("Firstname","Lastname","Gender","Address","Province","Username","Password"):
    tree.heading(col, text=col)
tree.pack(fill=BOTH, expand=1)
tree.bind("<Double-1>", OnSelected)
Read()
root.mainloop()'''
    # Always sync Activity 6 so the saved JSON reflects the latest resizable version
    _found = False
    for _e in activities_log:
        if _e["name"] == "Activity_6_student_crud.py":
            _e["content"] = crud_system_code
            save_activities()
            _found = True
            break
    if not _found:
...
user_entry = tk.Entry(login); user_entry.pack()
tk.Label(login, text="Password").pack(pady=5)
pass_entry = tk.Entry(login, show="*"); pass_entry.pack()
...
def compute_pay():
    try:
        rate = float(rate_entry.get()); days = float(days_entry.get())
        sss = float(sss_entry.get()); philhealth = float(philhealth_entry.get()); cash_adv = float(cash_entry.get())
        gross_pay = rate * days; total_deductions = sss + philhealth + cash_adv; net_pay = gross_pay - total_deductions
        gross_var.set(f"{gross_pay:.2f}"); deduct_var.set(f"{total_deductions:.2f}"); net_var.set(f"{net_pay:.2f}")
        cursor.execute("INSERT INTO payroll VALUES (?,?,?,?,?,?,?,?,?,?,?,?)",
            (emp_no_entry.get(), name_entry.get(), gender_var.get(), position_cb.get(),
             float(rate_entry.get()), float(days_entry.get()), gross_pay,
             float(sss_entry.get()), float(philhealth_entry.get()), float(cash_entry.get()),
             total_deductions, net_pay))
        conn.commit(); load_data()
...
def load_data():
    ...
    cursor.execute("SELECT emp_no,name,gender,position,rate,days,gross,sss,philhealth,cash,deductions,net FROM payroll")
    for row in cursor.fetchall(): tree.insert("", tk.END, values=row)

def delete_data():
    selected = tree.selection()
    if not selected: messagebox.showwarning("Select", "Please select a record!"); return
    if not messagebox.askyesno("Delete", "Are you sure?"): return
    for item in selected:
        values = tree.item(item, "values")
        cursor.execute("DELETE FROM payroll WHERE emp_no=? AND name=? AND gross=? AND net=?", (values[0], values[1], values[6], values[11]))
        tree.delete(item)
    conn.commit(); messagebox.showinfo("Deleted", "Record deleted!")

def clear_fields():
    for e in [emp_no_entry, name_entry, rate_entry, days_entry, sss_entry, philhealth_entry, cash_entry]: e.delete(0, tk.END)
    gross_var.set(""); deduct_var.set(""); net_var.set(""); gender_var.set(""); position_cb.set("")

def exit_app():
    if messagebox.askyesno("Exit", "Do you want to exit?"): root.destroy()
...
position_cb = ttk.Combobox(emp_frame, width=22, values=("Cashier","Bagger","Manager","Janitor","Guard")); position_cb.grid(row=3, column=1, pady=3)
tk.Label(emp_frame, text="Rate per Day:").grid(row=4, column=0, sticky="w")
rate_entry = tk.Entry(emp_frame, width=25); rate_entry.grid(row=4, column=1, pady=3)
tk.Label(emp_frame, text="Days Worked:").grid(row=5, column=0, sticky="w")
days_entry = tk.Entry(emp_frame, width=25); days_entry.grid(row=5, column=1, pady=3)
...
table_frame = tk.Frame(root); table_frame.pack(padx=10, pady=10, fill="both", expand=True)
columns = ("EMP_NO","NAME","GENDER","POSITION","RATE","DAYS","GROSSPAY","SSS","PHILHEALTH","CASH_ADVANCE","DEDUCTION","NET_PAY")
tree = ttk.Treeview(table_frame, columns=columns, show="headings")
for col in columns: tree.heading(col, text=col); tree.column(col, width=90)
tree.pack(fill="both", expand=True)

btn_frame = tk.Frame(root, bg="#f4f6f7"); btn_frame.pack(pady=10)
tk.Button(btn_frame, text="Compute", width=10, bg="#2ecc71", fg="white", command=compute_pay).grid(row=0, column=0, padx=5)
tk.Button(btn_frame, text="Clear",   width=10, bg="#fa9c04", fg="white", command=clear_fields).grid(row=0, column=1, padx=5)
tk.Button(btn_frame, text="Exit",    width=10, bg="#e74c3c", fg="white", command=exit_app).grid(row=0, column=2, padx=5)
...
def open_main_app():
    ...
    def compute_grade():
        try:
            prelim = float(entry_prelim.get()); midterm = float(entry_midterm.get()); final = float(entry_final.get())
            if not (0 <= prelim <= 100 and 0 <= midterm <= 100 and 0 <= final <= 100):
                messagebox.showerror("Error", "Grades must be between 0 and 100"); return None, None, None
            ...
            avg_result.config(text=f"{average:.2f}"); num_result.config(text=numerical); remark_result.config(text=remark)
            return average, numerical, remark
        except ValueError:
            messagebox.showerror("Error", "Please enter valid numbers"); return None, None, None

    def add_record():
        average, numerical, remark = compute_grade()
        if average is None: return
        tree.insert("", "end", values=(entry_id.get(), entry_name.get(), combo_course.get(), entry_subject.get(),
            entry_prelim.get(), entry_midterm.get(), entry_final.get(), f"{average:.2f}", numerical, remark))
        clear_fields()

    def clear_fields():
        for e in [entry_id, entry_name, entry_subject, entry_prelim, entry_midterm, entry_final]: e.delete(0, tk.END)
        combo_course.set(""); avg_result.config(text=""); num_result.config(text=""); remark_result.config(text="")

    def delete_record():
        sel = tree.selection()
        if not sel: messagebox.showwarning("Warning", "Please select a record."); return
        if messagebox.askyesno("Delete", "Are you sure?"): tree.delete(sel)
    ...
    def update_record():
        ...
        average, numerical, remark = compute_grade()
        if average is None: return
        tree.item(sel, values=(entry_id.get(), entry_name.get(), combo_course.get(), entry_subject.get(),
            entry_prelim.get(), entry_midterm.get(), entry_final.get(), f"{average:.2f}", numerical, remark))
        clear_fields()
    ...
    def load_record(event):
        ...
        entry_id.delete(0, tk.END); entry_id.insert(0, v[0])
        entry_name.delete(0, tk.END); entry_name.insert(0, v[1])
        combo_course.set(v[2])
        entry_subject.delete(0, tk.END); entry_subject.insert(0, v[3])
        entry_prelim.delete(0, tk.END); entry_prelim.insert(0, v[4])
        entry_midterm.delete(0, tk.END); entry_midterm.insert(0, v[5])
        entry_final.delete(0, tk.END); entry_final.insert(0, v[6])
        avg_result.config(text=v[7]); num_result.config(text=v[8]); remark_result.config(text=v[9])
    ...
    button_frame = tk.Frame(root, bg="#00FFFF"); button_frame.pack(pady=10)
    tk.Button(button_frame, text="Add / Compute",   width=15, bg="#2c7be5", fg="white", font=("Arial",10,"bold"), command=add_record).grid(row=0, column=0, padx=10)
    tk.Button(button_frame, text="Update Selected", width=15, bg="#ffc107", fg="white", font=("Arial",10,"bold"), command=update_record).grid(row=0, column=1, padx=10)
    tk.Button(button_frame, text="Clear Fields",    width=15, bg="#dc3545", fg="white", font=("Arial",10,"bold"), command=clear_fields).grid(row=0, column=2, padx=10)
    tk.Button(button_frame, text="Delete Selected", width=15, bg="#ff8800", fg="white", font=("Arial",10,"bold"), command=delete_record).grid(row=0, column=3, padx=10)
...
def check_login():
    if login_entry_user.get() == "admin" and login_entry_pass.get() == "1234":
        open_main_app()
    else:
...
tk.Label(login_win, text="STUDENT GRADING SYSTEM", font=("Arial",13,"bold"), bg="#00FFFF", fg="#1a3d7c").pack(pady=(20,5))
tk.Label(login_win, text="Please log in to continue", font=("Arial",9), bg="#00FFFF", fg="#333333").pack(pady=(0,15))
...
login_entry_user = ttk.Entry(login_frame, width=20); login_entry_user.grid(row=0, column=1, pady=5, padx=5); login_entry_user.focus()
tk.Label(login_frame, text="Password:", bg="#f7fbff", font=("Arial",10)).grid(row=1, column=0, sticky="w", pady=5)
login_entry_pass = ttk.Entry(login_frame, width=20, show="*"); login_entry_pass.grid(row=1, column=1, pady=5, padx=5)
login_entry_pass.bind("<Return>", lambda e: check_login())

login_error_label = tk.Label(login_win, text="", bg="#00FFFF", font=("Arial",9)); login_error_label.pack(pady=(5,0))
tk.Button(login_win, text="Login", width=15, bg="#2c7be5", fg="white", font=("Arial",10,"bold"), command=check_login).pack(pady=8)
login_win.mainloop()'''
